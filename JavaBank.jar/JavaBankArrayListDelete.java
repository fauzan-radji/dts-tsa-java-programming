...
public class JavaBankArrayListDelete extends JFrame {
  ...
  private void createUserInterface() {
    ...
    displayJTextArea.setText(
      "Welcome to Java Bank - There are currently no Accounts created"
    );
  ...
  private void CreateAccountJButtonActionPerformed(ActionEvent event) {
    ...
    if ((Name != "") & (Accountnum != 0)) {
      ...
      displayJTextArea.setText(
        Accounts.size() +
        " " +
        tempAccount.getaccountname() +
        " " +
        tempAccount.getaccountnum() +
        " " +
        tempAccount.getbalance()
      );
    } else {
      displayJTextArea.setText(
        "Both the Name field and Account Number must be completed"
      );
    }
